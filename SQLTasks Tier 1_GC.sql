...
the member name. */
SELECT DISTINCT CONAT(surname, firstname) AS member_name
FROM Members as m
LEFT JOIN Bookings AS b
ON m.memid = b.memid
LEFT JOIN Facilities AS f
ON b.facid = f.facid
WHERE b.facid < 2 AND m.memid > 0
...
SELECT name, SUM(cost) AS total_revenue
FROM
	...
	ON b.memid = m.memid
	LEFT JOIN Facilities AS f 
	ON b.facid = f.facid) AS booking_costs
GROUP BY name
